...
url = st.text_input('Enter File LInk', key = key_count)
...
sample_rate = st.number_input('Sampling rate (Hz)', value = 500, key = key_count)
key_count = key_count + 1
...
window_size = st.number_input('Enter the window size in seconds', value = 10, key = key_count)
key_count = key_count + 1
...
result = pd.DataFrame({'PAT': pat_list})
